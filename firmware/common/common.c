 ...
 * x264_log:
 ****************************************************************************/
// modification (custom implementation)
...
void *x264_malloc( int i_size )
{
  ...
  void *result;
  static uint32_t malloc_buffer[1966080] /* modification */ __attribute__ ((section(".sdram")));

  word_size = (i_size + 3) >> 2;
  
  result = (void *)&malloc_buffer[allocated];
  allocated += word_size;

//  debug_printf("%i %x %i\n", i_size, result, (sizeof(malloc_buffer) / sizeof(*malloc_buffer)) - allocated);
  return result;
}
